...
export async function PUT(req: NextRequest) {
  try {
    ...
    for (const field of quietHourFields) {
      ...
      if (fieldPath.length === 2 && preferencesUpdates[fieldPath[0]] && preferencesUpdates[fieldPath[0]][fieldPath[1]]) {
        const timeRegex = /^([01]?[0-9]|2[0-3]):[0-5][0-9]$/;
    ...
    if (preferencesUpdates.reminderTiming) {
      const { firstReminder, secondReminder, finalReminder } = preferencesUpdates.reminderTiming;
      
      if (firstReminder !== undefined && (firstReminder < 0 || firstReminder > 7)) {
        return NextResponse.json(
      ...
      if (secondReminder !== undefined && (secondReminder < 0 || secondReminder > 7)) {
        return NextResponse.json(
      ...
      if (finalReminder !== undefined && (finalReminder < 0 || finalReminder > 24)) {
        return NextResponse.json(
    ...
    if (preferencesUpdates.weeklyReportDay !== undefined) {
      if (preferencesUpdates.weeklyReportDay < 0 || preferencesUpdates.weeklyReportDay > 6) {
        return NextResponse.json(
          { error: 'Weekly report day must be between 0 (Sunday) and 6 (Saturday)' },
          { status: 400 }
        );
      }
    }
