  ...
  try {
    ...
    return NextResponse.json({
      summary: {
        ...
        openRate: successfulNotifications > 0 ? (openedNotifications / successfulNotifications * 100).toFixed(1) : '0',
        clickRate: openedNotifications > 0 ? (clickedNotifications / openedNotifications * 100).toFixed(1) : '0',
      },
      byType: stats.reduce((acc: any, stat: any) => {
        acc[stat._id] = {
          ...
          breakdown: stat.stats.reduce((breakdown: any, s: any) => {
            breakdown[s.status] = {
              count: s.count,
              avgDeliveryTime: s.avgDeliveryTime ? Math.round(s.avgDeliveryTime) : null
            };
            return breakdown;
          }, {})
        };
