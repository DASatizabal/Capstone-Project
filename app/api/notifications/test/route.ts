    ...
    switch (type) {
      ...
      case 'photo-rejection':
        result = await notificationService.sendPhotoRejectionNotification({
          ...
          requiresPhotoVerification: true,
          photoUrl: 'https://example.com/test-photo.jpg',
          rejectionReason: 'Please retake the photo with better lighting and make sure the whole area is visible.'
        });
        break;

      case 'daily-digest':
        result = await notificationService.sendDailyDigest(email, 'Test Family', {
          completedChores: [
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const stats = await reminderScheduler.getReminderStats(familyId || undefined);
