...
export const reminderScheduler = {
  ...
  scheduleRemindersForChore: async (choreId: string) => {
    try {
      ...
      firstReminderTime.setDate(dueDate.getDate() - preferences.reminderTiming.firstReminder);

      const secondReminderTime = new Date(dueDate);
      secondReminderTime.setDate(dueDate.getDate() - preferences.reminderTiming.secondReminder);

      const finalReminderTime = new Date(dueDate);
      finalReminderTime.setHours(dueDate.getHours() - preferences.reminderTiming.finalReminder);

      // Schedule reminders that are in the future
      const reminders = [];

      if (firstReminderTime > now && preferences.reminderTiming.firstReminder > 0) {
        reminders.push({
          type: 'first',
      ...
      if (secondReminderTime > now && preferences.reminderTiming.secondReminder > 0) {
        reminders.push({
          type: 'second',
      ...
      if (finalReminderTime > now && preferences.reminderTiming.finalReminder > 0) {
        reminders.push({
          type: 'final',
      ...
      console.log(`Scheduled ${reminders.length} reminders for chore ${choreId}`);
      return reminders;
    } catch (error) {
  ...
  processDueReminders: async () => {
    try {
      ...
      console.log(`Found ${choresDueForReminders.length} chores due for reminders`);
      ...
      for (const chore of choresDueForReminders) {
        try {
          ...
          if (!preferences.isNotificationAllowed()) {
            console.log(`Skipping reminder for user ${chore.assignedTo._id} due to quiet hours`);
            continue;
          }

          // Calculate days until due
          const daysUntilDue = Math.ceil((chore.dueDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
          ...
          } else if (daysUntilDue === preferences.reminderTiming.firstReminder) {
            shouldSendReminder = true;
          } else if (daysUntilDue === preferences.reminderTiming.secondReminder) {
            shouldSendReminder = true;
          } else if (daysUntilDue === 0 && (chore.dueDate.getTime() - now.getTime()) <= (preferences.reminderTiming.finalReminder * 60 * 60 * 1000)) {
            shouldSendReminder = true;
          }

          if (shouldSendReminder) {
            // Check rate limiting - don't send more than one reminder per day
            const lastReminderTime = preferences.lastNotifications?.choreReminder;
            if (lastReminderTime) {
              const hoursSinceLastReminder = (now.getTime() - lastReminderTime.getTime()) / (1000 * 60 * 60);
              if (hoursSinceLastReminder < 12) { // At least 12 hours between reminders
                console.log(`Rate limiting: Skipping reminder for chore ${chore._id}`);
                continue;
              }
            }

            // Send reminder
            const result = await notificationService.sendChoreReminderNotification({
            ...
            if (result.success) {
              ...
              console.log(`Sent reminder for chore ${chore._id} to ${chore.assignedTo.email}`);
            } else {
              console.error(`Failed to send reminder for chore ${chore._id}:`, result.error);
            }
          }
        } catch (error) {
          console.error(`Error processing reminder for chore ${chore._id}:`, error);
        }
      }
    } catch (error) {
  ...
  processDailyDigests: async () => {
    try {
      ...
      }).populate('user', 'email name').populate('family', 'name');

      console.log(`Found ${preferences.length} users for daily digest at ${currentTime}`);

      for (const pref of preferences) {
        try {
          // Check if we've already sent today's digest
          const lastDigestTime = pref.lastNotifications?.dailyDigest;
          if (lastDigestTime && lastDigestTime.toISOString().split('T')[0] === today) {
          ...
          const digest = {
            ...
            overdueChores: overdueChores.map(chore => ({
              ...
              daysOverdue: Math.ceil((now.getTime() - chore.dueDate.getTime()) / (1000 * 60 * 60 * 24))
            })),
            pendingApprovals,
            totalPointsEarned: completedChores.reduce((total, chore) => total + chore.points, 0)
          };
          ...
          if (result.success) {
            ...
            console.error(`Failed to send daily digest to ${pref.user.email}:`, result.error);
          }
        } catch (error) {
          console.error(`Error processing daily digest for user ${pref.user._id}:`, error);
        }
      }
    } catch (error) {
