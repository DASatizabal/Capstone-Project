...
notificationPreferencesSchema.index({ 'email.dailyDigest': 1, dailyDigestTime: 1 });
notificationPreferencesSchema.index({ 'email.weeklyReport': 1, weeklyReportDay: 1, weeklyReportTime: 1 });
...
notificationPreferencesSchema.methods.isNotificationAllowed = function(): boolean {
...
notificationPreferencesSchema.methods.updateLastNotification = function(type: string): Promise<INotificationPreferences> {
  if (this.lastNotifications && typeof this.lastNotifications === 'object') {
    (this.lastNotifications as any)[type] = new Date();
  }
  return this.save();
};
